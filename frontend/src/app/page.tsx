...
export default function LandingPage() {
  return (
    <div className="overflow-hidden pb-40 text-white">
      ...
      {/* How it Works - Adım Adım (Analiz Süreci) */}
      ...
      {/* Dashboard Preview (Yapay Zeka Modülü) */}
      <section className="relative mx-auto max-w-7xl px-6 mt-56 flex flex-col items-center">
        <div className="relative w-full max-w-5xl">
          <div className="rounded-[40px] border border-slate-200/60 bg-slate-50/80 p-4 shadow-2xl backdrop-blur-sm">
             <div className="overflow-hidden rounded-[32px] border border-slate-200/50 bg-white/40">
                <div className="bg-slate-200/30 px-6 py-4 flex items-center justify-between border-b border-slate-200/50">
                   <div className="flex gap-2">
                      <div className="size-3 rounded-full bg-rose-400" />
                      <div className="size-3 rounded-full bg-amber-400" />
                      <div className="size-3 rounded-full bg-emerald-400" />
                   </div>
                   <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">FinanceScout Analiz Paneli</div>
                </div>
                <div className="p-8 grid md:grid-cols-[280px_1fr] gap-8 text-left">
                   <div className="space-y-6">
                      <div className="p-5 rounded-2xl bg-white/60 border border-slate-200/60 space-y-5 shadow-sm">
                         <div>
                            <div className="text-[10px] font-bold text-slate-500 uppercase mb-2">Varlık Seçimi</div>
                            <div className="flex items-center justify-between h-10 w-full rounded-xl bg-sky-500/10 border border-sky-200 px-4">
                               <span className="text-sm font-bold text-slate-700">EURTRY=X</span>
                               <span className="text-[10px] text-sky-600 font-bold">DÖVİZ</span>
                            </div>
                         </div>
                         <div>
                            <div className="text-[10px] font-bold text-slate-500 uppercase mb-2">Analiz Kapsamı</div>
                            <div className="h-10 w-full rounded-xl bg-slate-100/50 border border-slate-200/50 px-4 flex items-center text-xs text-slate-500">
                               365 Gün Geçmiş
                            </div>
                         </div>
                      </div>
                      <div className="h-12 w-full rounded-xl bg-sky-500 text-white text-xs font-black flex items-center justify-center uppercase tracking-widest shadow-lg shadow-sky-500/20 hover:bg-sky-600 transition-all active:scale-[0.98]">Analizi Başlat</div>
                   </div>
                   <div className="space-y-6">
                      <div className="grid grid-cols-3 gap-4">
                         {[
                           {l: "HATA PAYI", v: "%1.2", c: "text-emerald-600"},
                           {l: "RİSK", v: "ORTA", c: "text-amber-600"},
                           {l: "GÜVEN", v: "%95", c: "text-sky-600"}
                         ].map(m => (
                           <div key={m.l} className="p-4 rounded-2xl bg-white/60 border border-slate-200/60 shadow-sm">
                              <div className="text-[9px] text-slate-500 font-bold uppercase tracking-tighter">{m.l}</div>
                      ...
                      <div className="relative h-48 w-full rounded-3xl bg-white/40 border border-slate-200/60 shadow-inner flex items-center justify-center p-6 overflow-hidden">
                         ...
                         <svg className="h-32 w-full mt-4" viewBox="0 0 400 100">
                            ...
                            {/* History Path */}
                            <path d="M0,85 Q50,45 100,65 T200,35 T300,55" fill="none" stroke="#94a3b8" strokeWidth="2" strokeOpacity="0.5" />
                            
                            {/* Forecast Path (Altın Sarısı) */}
                            <path d="M300,55 Q350,35 400,25" fill="none" stroke="#fbbf24" strokeWidth="3" strokeDasharray="6 3" />
                            
                            {/* Coordinate Points */}
                            <circle cx="100" cy="65" r="3" fill="#94a3b8" />
                            <circle cx="200" cy="35" r="3" fill="#94a3b8" />
                            <circle cx="300" cy="55" r="4" fill="#fbbf24" className="animate-pulse" />
                            <circle cx="400" cy="25" r="3" fill="#fbbf24" />
                         </svg>
                         <div className="absolute bottom-4 right-6 flex items-center gap-1.5 rounded-full bg-white/80 px-3 py-1 border border-slate-200 shadow-sm">
                            <div className="size-1.5 rounded-full bg-sky-500 animate-pulse" />
                            <span className="text-[9px] font-bold text-sky-600 uppercase">Hibrit Model Aktif</span>
                         </div>
                      </div>
                   </div>
                </div>
